...
int main( int  argc_ , char **argv_  )
{
  ...
  try {
    ...
    double event_time_stop = event_window;
    ...
    while (temp_signals.get_calo_signals().size() != 0 || temp_signals.get_geiger_signals().size() != 0)
      {
	//std::clog << " Calo size left in vector = " << temp_signals.get_calo_signals().size()
	//<< " Geiger size left in vector = " << temp_signals.get_geiger_signals().size() << std::endl;
	...
	// Predicate lambda function for calo
	auto below_calo_pred = [=](const datatools::handle<snemo::digitization::calo_signal> & ha) {
	  // std::clog << "Calo signal time = " << ha.get().get_signal_time() << " Event time stop = " << event_time_stop << std::endl;
	  return ha.get().get_signal_time() >= event_time_stop;
	};

	// Find calorimeter signals between [tstart; tstop]
	auto it_calo = std::find_if(temp_signals.get_calo_signals().begin(),
				    temp_signals.get_calo_signals().end(),
				    below_calo_pred);

	// Position - 1 is the last calorimeter signal in the window event
	std::size_t calo_position = it_calo - temp_signals.get_calo_signals().begin();

	for (std::size_t icalo = 0; icalo < calo_position; icalo++)
	  {
	    // temp_signals.get_calo_signals()[icalo].get().tree_dump(std::clog, "A calo signal #" + std::to_string(icalo));
	...
	// Remove these signals from the pool
	temp_signals.grab_calo_signals().erase(temp_signals.get_calo_signals().begin(),
					       it_calo);

	// Display after adding :
	for (std::size_t icalo = 0; icalo < event_signals.get_calo_signals().size(); icalo++)
	  {
	    // event_signals.get_calo_signals()[icalo].get().tree_dump(std::clog, "A calo signal #" + std::to_string(icalo) + " in event #" + std::to_string(event_counter));
	  }


	// Tracker signals :

	// Predicate lambda function for tracker
	auto below_geiger_pred = [=](const datatools::handle<snemo::digitization::geiger_signal> & ha) {
	  return ha.get().get_anode_avalanche_time() >= event_time_stop;
	};

	// Find calorimeter signals between [tstart; tstop]
	auto it_geiger = std::find_if(temp_signals.get_geiger_signals().begin(),
				      temp_signals.get_geiger_signals().end(),
				      below_geiger_pred);

	std::size_t geiger_position = it_geiger - temp_signals.get_geiger_signals().begin();

	for (std::size_t igeiger = 0; igeiger < geiger_position; igeiger++)
	  {
	...
	// Remove these signals from the pool
	temp_signals.grab_geiger_signals().erase(temp_signals.get_geiger_signals().begin(),
						 it_geiger);

	// Display after adding :
	for (std::size_t igeiger = 0; igeiger < event_signals.get_geiger_signals().size(); igeiger++)
	  {
	    // event_signals.get_geiger_signals()[igeiger].get().tree_dump(std::clog, "A geiger signal #" + std::to_string(igeiger) + " in event #" + std::to_string(event_counter));
	  }
	...
	// 	  << " Number of geiger in event : " << number_of_geiger_in_event << std::endl << std::endl;

	event_time_stop += event_window;

	event_counter++;
	//if (event_counter >= 10) break;

	// for (std::size_t icalo = 0; icalo < 10; icalo++)
	//   {
	//     temp_signals.get_calo_signals()[icalo].get().tree_dump(std::clog, "A calo signal #" + std::to_string(icalo));
	//   }
    ...
    // Begining trigger on the self triggering signals :

    // Trigger config from an external file, use default if no file
    ...
    std::clog << "Computions trigger decision for " << max_events << " events" << std::endl;
    for (std::size_t ievent = 0; ievent < max_events; ievent++)
      {
	...
	// Test if we consider all spurious signals = 1 'trigger event'
	// snemo::digitization::signal_data signal_data = calo_tracker_spurious_signals;
...
{
  int hit_count = 0;
  const double time_interval = 0.1 * CLHEP::second; // second
  const double calo_self_triggering_frequency = 1. / CLHEP::second; // Hertz
					      ...
					      snemo::digitization::signal_data & calo_tracker_spurious_signals_)
{
  int hit_count = 0;
  const double time_interval = 0.1 * CLHEP::second; // second
  const double geiger_self_triggering_frequency = 1. / CLHEP::second; // 'real' value = 10 Hertz
