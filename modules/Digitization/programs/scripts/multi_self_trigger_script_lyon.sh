...
echo "run_nbr time_interval calo_freq tracker_freq" > ${SELF_TRIGGER_DB}
...
#     CALO_FREQUENCY=${i}
#     TRACKER_FREQUENCY=${i}
#     echo "run_${INPUT_SD_RUN} time_int_${TIME_INTERVAL} calo_freq_${CALO_FREQUENCY} tracker_freq_${TRACKER_FREQUENCY}"
#     echo "run_${INPUT_SD_RUN} time_int_${TIME_INTERVAL} calo_freq_${CALO_FREQUENCY} tracker_freq_${TRACKER_FREQUENCY}" >> ${SELF_TRIGGER_DB}

#     # qsub -V -P P_nemo -l h_cpu=12:00:00,fsize=8000M,sps=1 -q huge@@C6320IImc -e ${ANALYSIS_LOG_WORK_DIR} -o ${ANALYSIS_LOG_WORK_DIR} self_trigger_script.sh -r ${i} -s ${TIME_INTERVAL} -c ${CALO_FREQUENCY} -t ${TRACKER_FREQUENCY}
#     qsub -V -P P_nemo -l h_cpu=12:00:00,fsize=8000M,sps=1 -q huge@@C6320IImc -e ${ANALYSIS_LOG_WORK_DIR} -o ${ANALYSIS_LOG_WORK_DIR} self_trigger_script.sh -r ${i} -s ${TIME_INTERVAL} -c ${CALO_FREQUENCY} -t ${TRACKER_FREQUENCY}
# done
