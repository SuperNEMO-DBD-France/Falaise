								...
								if (word.length() > 1)
									{
										if(word[1] == '@')
											{
												DT_THROW_IF(header_done, std::logic_error, "Metadatas outside header at line '" <<  line << "' ! ");

												typedef std::vector<std::string> split_vector_type;
												split_vector_type key_value_vect;
												boost::split(key_value_vect, line, boost::is_any_of("="), boost::token_compress_off);
												DT_THROW_IF(key_value_vect.size() != 2, std::logic_error, "Key = value format error at line '"<< line << "' ! ");
												...
												if (key == "#@address_size")
												...
												else if (key == "#@registered_value")
													{
														...
														registered_values.insert(std::pair<std::string, std::bitset<DataSize> >(label, bitset_data));
													}
				  ...
				  std::bitset<AddressSize> address_bitset;
					...
					if (registered_values.find(data_string) != registered_values.end())
					...
					else
						{
							...
							dd_iss >> data_bitset;
							DT_THROW_IF(!dd_iss, std::logic_error, "Invalid format for data bitset '"<< data_string << "' ! ");
						}
