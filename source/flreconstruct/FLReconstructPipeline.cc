  ...
  falaise::exit_code do_pipeline(const FLReconstructParams& flRecParameters)
  {
    ...
    try {
      ...
      datatools::service_manager recServices("flReconstructionServices",
                                             "SuperNEMO Reconstruction Services",
                                             servicesFlags);
      ...
      DT_LOG_DEBUG(flRecParameters.logLevel, "Number of entries  = " << recInput->get_source().get_number_of_entries());
      DT_LOG_DEBUG(flRecParameters.logLevel, "Number of metadata = " << recInput->get_source().get_number_of_metadata());

      // Output metadata management:
      DT_LOG_DEBUG(flRecParameters.logLevel,"Building output metadata...");
      datatools::multi_properties flRecMetadata("name",
                                                "type",
      ...
      while (true) {
        ...
        DT_THROW_IF(pStatus == dpp::base_module::PROCESS_INVALID, std::logic_error,
                    "Bug!!! Module '" << pipeline->get_name() << "' did not return a valid processing status!");
