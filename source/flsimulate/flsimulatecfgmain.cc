...
namespace FLSimulateConfig {
  ...
  void do_help(const boost::program_options::options_description& od)
  {
    std::cout << "flsimulate-configure (" << falaise::version::get_version() << ") : SuperNEMO simulation profile configuration program\n";
    std::cout << "\n";
  ...
  void do_cldialog(int argc, char *argv[], FLSimulateConfigureCommandLine & clArgs)
  {
    ...
    optDesc.add_options()
      ...
      ("setup-tag,t",
       bpo::value<std::string>(&clArgs.simulationSetupUrn)->value_name("urn")->default_value(FLSimulate::default_simulation_setup()),
       "tag of the simulation setup on top of which the generated variant profile should be applied")
      ...
      ("input-profile,i",
       ...
       "file from which to load a variant profile used as a base for the generated profile")

      ("setting,s",
       bpo::value<std::vector<std::string>>(&clArgs.inputVariantSettings)->value_name("setting"),
      ...
      ("gui",
       ...
       "activate the variant service's GUI from which to browse and edit the variant profile")
    ...
    if (vMap.count("input-profile-tag") && vMap.count("input-profile")) {
      std::cerr << "[OptionsException] " << "Cannot use simultaneously '--input-profile-tag' and '--input-profile'" << std::endl;
      throw FLSimulate::FLDialogOptionsError();
    }
  ...
  void do_configure(int argc, char *argv[], FLSimulateConfigureParams& flSimCfgParameters)
  {
...
#else
    DT_LOG_WARNING(flSimCfgParameters.logLevel, "The current version of Falaise does not support GUI mode!");
#endif // DATATOOLS_WITH_QT_GUI == 1
    ...
    DT_THROW_IF(flSimCfgParameters.variantServiceConfig.profile_load.empty(),
                std::logic_error,
  ...
  falaise::exit_code do_flsimulate_config(int argc, char *argv[])
  {
    ...
    if (!flSimCfgParameters.variantServiceConfig.logging.empty()) {
      variantService.set_logging(datatools::logger::get_priority(flSimCfgParameters.variantServiceConfig.logging));
    }
    try {
